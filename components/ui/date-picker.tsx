...
export const DatePicker: React.FC<DatePickerProps> = ({
  ...
  const startDate = minDate || new Date(1980, 0);
  const endDate = maxDate || new Date(2030, 11);
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-auto p-0 bg-background border-border shadow-lg" align="start">
        <Calendar
          ...
          initialFocus
          className="overflow-hidden rounded-md border-0 p-2"
          classNames={{
    ...
    <div className="relative">
      ...
      {isYearView && (
        <div className="bg-background absolute inset-0 z-20 -mx-2 -mb-2">
          <ScrollArea ref={scrollAreaRef} className="h-full max-h-[300px]">
            {years.map((year) => {

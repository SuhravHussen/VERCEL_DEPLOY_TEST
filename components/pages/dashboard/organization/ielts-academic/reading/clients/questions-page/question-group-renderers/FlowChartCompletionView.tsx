...
export function FlowChartCompletionView({
  ...
  return (
    <div className="rounded-md border bg-card p-3 sm:p-4">
      ...
      {/* Display flowchart based on type */}
                  ...
                  <div className="absolute inset-0 pointer-events-none">
                    {typedGroup.inputPositions.map(
                      (
                        ...
                        <div
                          key={index}
                          className="absolute w-6 h-6 bg-blue-500 text-white rounded-full flex items-center justify-center text-xs font-bold shadow-lg border-2 border-white"
                          style={{
                            left: `${position.x}%`,
                            top: `${position.y}%`,
      ...
      {typedGroup.chartType === "text" &&
        ...
        typedGroup.textSteps.length > 0 && (
          <div className="mb-3 sm:mb-4">
            <div className="flex items-center gap-2 mb-2">
              <Type className="h-3 w-3 sm:h-4 sm:w-4 text-primary" />
              <h4 className="font-medium text-sm sm:text-base">
                Flow Chart Steps
              </h4>
            </div>
                    ...
                    >
                      ...
                      <div className="flex-grow">
                        <div className="text-sm">
                          ...
                          {step.isGap && (
                            <span className="inline-flex items-center justify-center bg-yellow-100 border-2 border-dashed border-yellow-400 px-3 py-1 rounded text-xs font-medium text-yellow-800">
                              GAP {index + 1}
                            </span>
                          )}
                          {step.textAfter && (
      ...
      {!typedGroup.chartType && typedGroup.chartImage && (
        <div className="mb-3 sm:mb-4">
          <div className="flex items-center gap-2 mb-2">
            <ImageIcon className="h-3 w-3 sm:h-4 sm:w-4 text-primary" />
            <h4 className="font-medium text-sm sm:text-base">Flow Chart</h4>
          </div>

...
export function GenericAllExamCard({
  exam,
  onViewDetails,
  onDelete,
  isDeleting = false,
    ...
    <>
      ...
      >
        ...
        <CardContent className="flex-grow space-y-3">
          ...
          {/* Speaking Sessions (if available) */}
          {exam.speaking_group &&
            exam.speaking_group.time_windows.length > 0 && (
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                <MapPin className="h-4 w-4" />
                <span>
                  {exam.speaking_group.time_windows.length} speaking session
                  {exam.speaking_group.time_windows.length > 1 ? "s" : ""}{" "}
                  available
                </span>
              </div>
            )}

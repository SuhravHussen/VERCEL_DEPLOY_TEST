...
import IELTSReadingQuestionGradingRenderer from "./ielts-reading-question-grading-renderer";

interface ReadingGradingRendererProps {
...
export default function ReadingGradingRenderer({
  test,
  userAnswers,
}: ReadingGradingRendererProps) {
  ...
  const [calcMode, setCalcMode] = useState<"academic" | "general">("academic");

  // Process reading test with question numbering
  const testSections = [test.section_one, test.section_two, test.section_three];
  const { numberedSections, totalQuestions, passageStats } =
    addQuestionNumbering(testSections);
  ...
  // Reading band calculator (Academic vs General)
  const computeReadingBand = (
    correct: number,
  ...
  const computedBand = useMemo(
    () => computeReadingBand(calcCorrect, calcMode),
    [calcCorrect, calcMode]
  );
        ...
        section.questions.forEach((questionGroup) => {
          questionGroup.questions?.forEach((question: QuestionWithNumber) => {
            ...
            if (qNum === questionNumber) {
              // Handle different question types with different answer fields
  ...
  }, [allQuestionNumbers, manualGrades, totalQuestions]);
  ...
  const renderQuestionGroup = (
    ...
    return (
      <IELTSReadingQuestionGradingRenderer
      ...
      <div className="flex justify-end gap-2">
        ...
        >
          ...
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              <DialogTitle>Reading Grade Calculator</DialogTitle>
              <DialogDescription>
                Choose test type and enter correct answers to estimate the band
                score.
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4">
              <div className="flex items-center gap-2">
                <Button
                  size="sm"
                  variant={calcMode === "academic" ? "default" : "outline"}
                  className="h-8 px-3 text-xs"
                  onClick={() => setCalcMode("academic")}
                >
                  Academic
                </Button>
                <Button
                  size="sm"
                  variant={calcMode === "general" ? "default" : "outline"}
                  className="h-8 px-3 text-xs"
              ...
              <div>
                <div className="text-xs font-medium mb-2">
                  Conversion table (
                  {calcMode === "academic" ? "Academic" : "General Training"})
                </div>
                  ...
                  <div className="max-h-56 overflow-auto">
                    ...
                    {(calcMode === "academic"
                      ...
                      : [
                          { range: "40", band: "9.0" },
                          { range: "39", band: "8.5" },
                          { range: "37–38", band: "8.0" },
                          { range: "36", band: "7.5" },
                          { range: "34–35", band: "7.0" },
                          { range: "32–33", band: "6.5" },
                          { range: "30–31", band: "6.0" },
                          { range: "27–29", band: "5.5" },
                          { range: "23–26", band: "5.0" },
                          { range: "19–22", band: "4.5" },
                          { range: "15–18", band: "4.0" },
                        ]
                    ).map((row) => (
                      <div
                        key={row.range}
      ...
      {isDetailedView ? (
        <>
          ...
          {numberedSections.map((section, sectionIndex) => {
            const passageStat = passageStats[sectionIndex];
